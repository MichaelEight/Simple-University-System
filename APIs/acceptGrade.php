...
try {
    if (!empty($token)) {
        ...
        if ($tokenData) {
            // Assuming that only students can accept grades
            if ($tokenData['role'] === "student") {
                // Token is valid; proceed to update the grade
                $stmt = $conn->prepare("UPDATE Grades SET acceptance_state = 'accepted' WHERE grade_id = :gradeId AND student_id = :studentId");
                $stmt->bindParam(':gradeId', $gradeId, PDO::PARAM_INT);
                $stmt->bindParam(':studentId', $tokenData['user_id'], PDO::PARAM_INT);
                $stmt->execute();

                echo json_encode(['success' => 'Grade accepted']);
            } else {
